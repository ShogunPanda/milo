...
async function main() {
  const milo = await import(`./lib/${process.argv[2]}/milo.js`)
  ...
  const buffer = milo.malloc(4096, 1) >>> 0
  ...
  console.log(`{ "pos": ${parser.position}, "consumed": ${consumed}, "state": "${milo.States[parser.state]}" }`)
