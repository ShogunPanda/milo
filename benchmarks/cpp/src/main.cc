...
static std::string decode_fixture(const std::string& raw) {
  ...
  for (size_t i = 0; i < raw.size();) {
    if (raw[i] == '\n') {
      i++;
    } else if (i + 3 < raw.size() && raw[i] == '\\' && raw[i + 1] == 'r' && raw[i + 2] == '\\' && raw[i + 3] == 'n') {
      decoded.push_back('\r');
      decoded.push_back('\n');
      i += 4;
    } else {
...
static void validate_llhttp(const Fixture& fixture) {
  ...
  llhttp_t parser;
  llhttp_init(&parser, fixture.is_request ? HTTP_REQUEST : HTTP_RESPONSE, &settings);

  const llhttp_errno_t error = llhttp_execute(&parser, fixture.payload.data(), fixture.payload.size());
  if (error != HPE_OK) {
    std::cerr << "llhttp failed to parse fixture " << fixture.name << ": " << llhttp_errno_name(error) << std::endl;
    std::exit(1);
  }
}
...
static Result benchmark_milo(const Fixture& fixture) {
  ...
  return {"milo-cpp", format_number(iterations), format_number(bytes_per_second / (1024.0 * 1024.0)),
          format_number(static_cast<double>(iterations) / seconds), bytes_per_second};
}
...
static Result benchmark_llhttp(const Fixture& fixture) {
  ...
  if (errors != HPE_OK) {
    std::cerr << "llhttp failed while benchmarking fixture " << fixture.name << std::endl;
    std::exit(1);
  }
  ...
  const double seconds = elapsed.count();
  const double bytes_per_second = static_cast<double>(total) / seconds;
  return {"llhttp-cpp", format_number(iterations), format_number(bytes_per_second / (1024.0 * 1024.0)),
          format_number(static_cast<double>(iterations) / seconds), bytes_per_second};
}

static void print_separator(size_t parser_width, size_t iterations_width, size_t mb_width, size_t ops_width) {
  std::cout << "| " << std::string(parser_width, '-') << " | " << std::string(iterations_width, '-') << " | "
            << std::string(mb_width, '-') << " | " << std::string(ops_width, '-') << " |" << std::endl;
}

static void print_results(const Fixture& fixture, const std::vector<Result>& results) {
  std::vector<Result> sorted = results;
  std::sort(sorted.begin(), sorted.end(),
            [](const Result& a, const Result& b) { return a.bytes_per_second < b.bytes_per_second; });
  ...
  std::cout << "| " << pad_right("Parser", parser_width) << " | " << pad_left("Iterations", iterations_width) << " | "
            << pad_left("MB/s", mb_width) << " | " << pad_left("Ops/s", ops_width) << " |" << std::endl;
  print_separator(parser_width, iterations_width, mb_width, ops_width);

  for (const Result& result : sorted) {
    std::cout << "| " << pad_right(result.parser, parser_width) << " | "
              << pad_left(result.iterations, iterations_width) << " | " << pad_left(result.mb_per_second, mb_width)
              << " | " << pad_left(result.ops_per_second, ops_width) << " |" << std::endl;
  }
